...
export async function up({ payload }: MigrateUpArgs): Promise<void> {
  // Check if university_templates table exists
  ...
  // Create university_templates table if it doesn't exist
  ...
  // Check if universities table exists and add missing phone column
  ...
  // Check and fix payload_locked_documents__rels table for universities_id column
  ...
  // Get relation tables that might need universities_id column
  const relationTablesResult = await payload.db.drizzle.execute(sql`
    SELECT table_name 
  ...
  // Add universities_id column to other relation tables if missing
  if (Array.isArray(relationTablesResult)) {
    for (const row of relationTablesResult) {
      ...
      if (!universitiesIdExists[0]?.exists) {
        ...
        await payload.db.drizzle.execute(sql`
          DO $$ BEGIN
            ALTER TABLE ${sql.identifier(tableName)}
            ADD CONSTRAINT ${sql.identifier(`${tableName}_universities_fk`)}
            FOREIGN KEY ("universities_id") REFERENCES "universities"("id") ON DELETE CASCADE;
          EXCEPTION
            WHEN duplicate_object THEN null;
          END $$;
...
export async function down({ payload }: MigrateDownArgs): Promise<void> {
  // Remove phone column from universities table
  await payload.db.drizzle.execute(sql`
    ALTER TABLE "universities" 
    DROP COLUMN IF EXISTS "phone";
  `);

  // Remove universities_id column from payload_locked_documents__rels table
  await payload.db.drizzle.execute(sql`
    ALTER TABLE "payload_locked_documents__rels" 
    DROP COLUMN IF EXISTS "universities_id";
  `);

  // Remove university_templates_id column from payload_locked_documents__rels table
  await payload.db.drizzle.execute(sql`
    ALTER TABLE "payload_locked_documents__rels" 
